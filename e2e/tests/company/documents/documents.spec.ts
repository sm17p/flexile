...
test.describe("Documents search functionality", () => {
  ...
  test("displays a link to invite a lawyer", async ({ page }) => {
    const { company } = await companiesFactory.createCompletedOnboarding();
    const { user: admin } = await usersFactory.create();
    await companyAdministratorsFactory.create({
      companyId: company.id,
    ...
    await expect(link).toHaveAttribute("href", "/settings/administrator/members?addMembers=true");

    await link.click();

    await page.waitForURL("/settings/administrator/members?addMembers=true");
