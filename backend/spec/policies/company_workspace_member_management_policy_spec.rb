...
RSpec.describe CompanyWorkspaceMemberManagementPolicy do
  ...
  describe "when user is a company administrator" do
    ...
    it "validates role arrays correctly" do
      expect(policy.can_manage_all_roles?(["admin", "lawyer"])).to be true
      expect(policy.can_manage_all_roles?(["admin", "invalid"])).to be false
      expect(policy.can_manage_all_roles?([])).to be false
      expect(policy.can_manage_all_roles?(nil)).to be false
    end
  end
